...
df = pd.read_excel('NKT_copy.xlsx', sheet_name=s_name, header=None)
...
stages = [
    ...
    {'name': 'Ép thủy lực', 'ngay': 37, 'nguoi1': 35, 'nguoi2': 36, 'tinhTrang': 34, 'apSuat': 33, 'maBoTP': 38, 'maBoHong': 39},
    {'name': 'Đóng gói', 'ngay': 42, 'nguoi1': 41, 'khoang': 40},
...
for idx in range(4, len(df)):
    ...
    for st in stages:
        ...
        # Filter for June 1 to June 12
        if ngay >= '2026-06-01' and ngay <= '2026-06-12':
            tinhTrang = clean_str(row[st.get('tinhTrang', -1)]) if 'tinhTrang' in st else 'Đạt'
            if tinhTrang == 'OK':
                tinhTrang = 'Đạt'
            if tinhTrang == '':
                tinhTrang = 'Đạt'
                
            # Ma bo depends on operation and status
            maBo = ''
            if st['name'] == 'Đầu vào':
                maBo = clean_str(row[st['maBo']])
            elif st['name'] == 'Ép thủy lực':
                if tinhTrang == 'Đạt':
                    maBo = clean_str(row[st['maBoTP']])
                else:
                    maBo = clean_str(row[st['maBoHong']])
...
for k, pipes in groups.items():
    ...
    final_data.append({
        ...
        'nguoi2': n2,
        'tinhTrang': tt,
        'ghiChu': 'Imported from Excel (Tháng 6)'
    })

with open('imported_june.json', 'w', encoding='utf-8') as f:
    json.dump(final_data, f, ensure_ascii=False, indent=2)

print(f"Extracted {len(final_data)} grouped records from Excel.")
