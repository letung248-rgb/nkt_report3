...
new_headers = 'const headers = ["ID", "Ngày", "Ca", "Nguyên công", "Số lần rửa", "Số lượng ống", "Số ống chi tiết", "Loại xử lý", "Loại ống", "Mã bộ", "Khoang chứa ống", "Từ giếng", "Từ giàn", "Số BBGN", "Hồ sơ giếng", "Áp suất", "Người TH 1", "Người TH 2", "Tình trạng", "Ghi chú"];'
...
new_block = r'''const nguyenCongFull = r.nguyenCong + (r.dongHoNuoc ? ' [ĐH: ' + r.dongHoNuoc + ']' : '');
            ...
            if (pipes.length === 0) {
                ...
                wsData.addRow([
                    idStr, formatDate(r.ngay), r.ca, nguyenCongFull, soLanRuaStr, r.soOngCount, r.soOngText,
                    r.loaiXl, r.loaiOng, resolvedMaBo, r.khoang, tuGieng, tuGian, soBBGN, hoSoGieng,
                ...
                for (let pipe of pipes) {
                    ...
                    wsData.addRow([
                        idStr, formatDate(r.ngay), r.ca, nguyenCongFull, soLanRuaStr, 1, pipe,
                        r.loaiXl, r.loaiOng, displayMaBo, r.khoang, tuGieng, tuGian, soBBGN, hoSoGieng,
                        r.apSuat || '', r.nguoi1, r.nguoi2, r.tinhTrang, (r.ghiChu || '').replace(/\n/g, ' ')
                    ]);
                    rowIndex++;
                }
